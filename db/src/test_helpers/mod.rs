...
use cargo_autoinherit_test_config::DatabaseConfig;
...
/// This function is automatically called by the [`cargo-autoinherit-test-macros::db_test`] macro. The return connection pool is passed to the test case via the [`cargo-autoinherit-test-macros::DbTestContext`].
#[allow(unused)]
...
///
/// This function is automatically called by the [`cargo-autoinherit-test-macros::db_test`] macro. It ensures test-specific database are cleaned up after each test run so we don't end up with large numbers of unused databases.
