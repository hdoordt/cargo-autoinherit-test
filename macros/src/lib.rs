//! The cargo_autoinherit_test-macros crate contains the `test` and `db_test` macros.
...
/// ```
///
/// Test functions marked with this attribute receive a [`cargo_autoinherit_test-web::test_helpers::TestContext`] struct via which they get access a preconfigured instance of the application. The application instance is extended with convenience methods for making requests from the test.
...
pub fn test(_: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let setup = quote! {
        let context = cargo_autoinherit_test_web::test_helpers::setup().await;
    };
...
/// ```
///
/// Test functions marked with this attribute receive a [`cargo_autoinherit_test-web::test_helpers::DbTestContext`] struct via which they get access a preconfigured instance of the application as well as a pool of database connections. The connection pool is connected to the dedicated database for this single test case. The application instance is configured to be connected to the same database so that data created in the test is accessible to the application and vice versa (see in the example how a task is created in the task, which the application reads and responds with as JSON). That allows full-stack testing without interfering with other tests.
///
...
pub fn db_test(_: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let teardown = quote! {
        cargo_autoinherit_test_web::test_helpers::teardown(context).await;
    };
