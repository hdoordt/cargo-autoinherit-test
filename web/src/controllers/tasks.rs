...
use cargo_autoinherit_test_db::{entities::tasks, transaction};
use payloads::*;
use tracing::info;
use uuid::Uuid;
...
///
/// This function creates a task in the database (see [`cargo_autoinherit_test_db::entities::tasks::create`]) based on a [`cargo_autoinherit_test_db::entities::tasks::TaskChangeset`] (sent as JSON). If the task is created successfully, a 201 response is returned with the created [`cargo_autoinherit_test_db::entities::tasks::Task`]'s JSON representation in the response body. If the changeset is invalid, a 422 response is returned.
#[axum::debug_handler]
...
///
/// This function creates multiple tasks in the database (see [`cargo_autoinherit_test_db::entities::tasks::create`]) based on [`cargo_autoinherit_test_db::entities::tasks::TaskChangeset`]s (sent as JSON). If all tasks are created successfully, a 201 response is returned with the created [`cargo_autoinherit_test_db::entities::tasks::Task`]s' JSON representation in the response body. If any of the passed changesets is invalid, a 422 response is returned.
///
...
///
/// This function reads all [`cargo_autoinherit_test_db::entities::tasks::Task`]s from the database (see [`cargo_autoinherit_test_db::entities::tasks::load_all`]) and responds with their JSON representations.
#[axum::debug_handler]
...
pub async fn read_all(
    State(app_state): State<AppState>,
) -> Result<Json<ReadAllResponsePayload>, Error> {
...
///
/// This function reads one [`cargo_autoinherit_test_db::entities::tasks::Task`] identified by its ID from the database (see [`cargo_autoinherit_test_db::entities::tasks::load`]) and responds with its JSON representations. If no task is found for the ID, a 404 response is returned.
#[axum::debug_handler]
...
///
/// This function updates a task identified by its ID in the database (see [`cargo_autoinherit_test_db::entities::tasks::update`]) with the data from the passed [`cargo_autoinherit_test_db::entities::tasks::TaskChangeset`] (sent as JSON). If the task is updated successfully, a 200 response is returned with the created [`cargo_autoinherit_test_db::entities::tasks::Task`]'s JSON representation in the response body. If the changeset is invalid, a 422 response is returned.
#[axum::debug_handler]
...
///
/// This function deletes one [`cargo_autoinherit_test_db::entities::tasks::Task`] identified by the entity's id from the database (see [`cargo_autoinherit_test_db::entities::tasks::delete`]) and responds with a 204 status code and empty response body. If no task is found for the ID, a 404 response is returned.
#[axum::debug_handler]
...
mod payloads {
    use cargo_autoinherit_test_db::entities::tasks::{Task, TaskChangeset};
    use cargo_autoinherit_test_macros::{batch_request_payload, request_payload, response_payload};
