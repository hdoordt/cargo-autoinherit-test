//! The cargo_autoinherit_test_web crate contains the application's web interface which mainly are controllers implementing HTTP endpoints. It also includes the application tests that are black-box tests, interfacing with the application like any other HTTP client.

use anyhow::Context;
use axum::serve;
use cargo_autoinherit_test_config::{get_env, load_config, Config};
...
///
/// 1. Determine the environment the application is running in (see [`cargo_autoinherit_test_config::get_env`])
/// 2. Load the configuration (see [`cargo_autoinherit_test_config::load_config`])
