...
pub enum Error {
    ...
    Database(#[from] cargo_autoinherit_test_db::Error),
...
impl IntoResponse for Error {
    fn into_response(self) -> axum::response::Response {
        match self {
            Error::Database(cargo_autoinherit_test_db::Error::NoRecordFound) => {
                StatusCode::NOT_FOUND.into_response()
            }
            Error::Database(cargo_autoinherit_test_db::Error::ValidationError(e)) => {
                validation_error(e).into_response()
            }
            Error::Database(cargo_autoinherit_test_db::Error::DbError(e)) => {
